...
시스템에서 복잡한 비지니스 요구사항을 만족하기위해서는, 하나의 명령(Command)이 실행 됐을 때 여러 서비스 컴포넌트 간 데이터 일관성을 위해 다양한 방식의 통신이 발생하고 이를 제어하기 위해선 하나의 유즈케이스에 많은 의존성이 필요해질 때가 있다.
...
>     
>     * 위 조건을 만족하기 위해, KOS에서는 여러 도메인 B.C 에서 변경이 발생한 사건들(Domain Events)을 발행하여 Downstream B.C 에서 요구사항에 맞추어 처리하고있다.
>         
